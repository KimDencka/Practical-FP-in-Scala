...
object Generators {
  ...
  val brandParamGen: Gen[BrandNameParam] =
    arbitrary[NonEmptyString].map(x => BrandNameParam(x))

  val createItemParamGen: Gen[CreateItemParam] =
    for {
      n <- arbitrary[NonEmptyString].map(x => ItemNameParam(x))
      d <- arbitrary[NonEmptyString].map(x => ItemDescriptionParam(x))
      p <- arbitrary[String Refined ValidBigDecimal].map(x => PriceParam(x))
      b <- brandIdGen
      c <- categoryIdGen
